...
async def get_file(message: types.Message):
    ...
    if message.from_user.id in video_get_accaunt:
        if settings.state_of_keyboard== 'YouTube':
            ...
            if x[5]=="Аудио":
                await bot.send_voice(int(x[0]),message.voice.file_id,caption=x[3],duration=message.audio.duration,reply_markup=client_kb.kb_main_menu)
                work_with_db.youtube_videos(x[3], x[2], x[1], x[4], message.voice.file_id)
            else:
                await bot.send_video(int(x[0]),message.video.file_id,message.video.duration,thumb=x[6],caption=x[3],supports_streaming=True,reply_markup=client_kb.kb_main_menu)
